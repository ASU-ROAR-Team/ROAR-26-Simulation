    ...
    def __init__(self, npz_path: str, min_terrain_height: float = 0.15,
        ...
        if not os.path.isfile(npz_path):
            raise FileNotFoundError(
                ...
                "Provide --heightmap pointing to the terrain heightmap NPZ."
            )
...
def generate_obstacle_data(
    ...
    # Deterministic generation for reproducible benchmark worlds.
    if seed is not None:
        random.seed(seed)
    ...
    if heightmap_path is None:
        raise FileNotFoundError(
            "Could not find terrain heightmap. Pass --heightmap explicitly."
        )
    ...
    if target_rock_count is not None:
        num_rocks = max(0, int(target_rock_count))
        density = num_rocks / area
    else:
    ...
    print(f"  Deadend Count        : {deadend_count}")
    print(f"  Reserved Corridor    : {corridor_width:.2f} m")
    print(f"  Corridor Centre X    : {corridor_center_x:.2f} m")
    ...
    # ------------------------------------------------------------
    # Dead-end / barrier formations
    #
    # These intentionally make the primary corridor harder, but
    # do NOT place a complete wall across the whole map.
    # ------------------------------------------------------------
    effective_deadends = deadend_count

    if deadends and effective_deadends <= 0:
        effective_deadends = 1
    ...
    for barrier_index in range(
        min(effective_deadends, len(barrier_y_positions))
    ):
        by = barrier_y_positions[barrier_index]

        # Alternate the gap left/right so the rover has to
        # change route rather than follow a straight line.
        if barrier_index % 2 == 0:
            barrier_xs = [-3.0, -1.5, 0.0, 1.5]
        else:
            barrier_xs = [-1.5, 0.0, 1.5, 3.0]

        for bx in barrier_xs:
            if len(rock_data_list) >= num_rocks:
                break

            if not sampler.is_valid_terrain(bx, by):
                continue
    ...
    for idx in range(start_idx, num_rocks + 1):
        ...
        for _ in range(max_attempts):
            ...
            # --------------------------------------------------
            # Reserved navigation corridor
            #
            # The course is primarily aligned with the Y axis.
            # Random rocks are kept outside a central corridor.
            # Dead-end formations above may intentionally intrude
            # into it to create detours.
            # --------------------------------------------------
            if corridor_width > 0.0:
                half_corridor = corridor_width / 2.0

                if abs(cand_x - corridor_center_x) < half_corridor:
                    continue
    ...
    with open(info_file, "w") as f:
        ...
        f.write(f"Deadend Count   : {effective_deadends}\n")
        f.write(f"Target Rocks    : {num_rocks}\n")
        f.write(f"Corridor Width  : {corridor_width} m\n")
        f.write(f"Corridor X      : {corridor_center_x} m\n")
        f.write(f"Seed            : {seed}\n")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Enable dead-end barrier formations",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Explicit number of rocks to generate. "
            "Overrides density-based rock count."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Width in metres of the reserved central "
            "navigation corridor."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="X centre of the reserved navigation corridor.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of dead-end barrier formations.",
    )
    parser.add_argument(
...
def main():
    ...
    generate_obstacle_data(
        ...
        target_rock_count=args.target_rock_count,
        seed=args.seed,
        corridor_width=args.corridor_width,
        corridor_center_x=args.corridor_center_x,
        deadend_count=args.deadend_count,
