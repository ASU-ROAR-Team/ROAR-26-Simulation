...
## ❌ Mission Build Rejections: Dead-Ends vs Short-End Rejects

`waypointBuilder.build_waypoints` logs a summary line each run, e.g.:

```
build_waypoints: 1000 attempts | 91 accepted | 792 dead‑ends | 117 short‑end rejects
```

Each attempt tries to grow one 5-point mission by randomly walking from candidate to candidate, and can fail in one of two distinct ways before an attempt becomes an accepted mission:

* **Dead-end:** the walk stalls partway through because no remaining candidate satisfies *all* of: distance from the current point falls within `[min_spacing_cells, max_spacing_cells]`, the candidate hasn't already been used in this mission, and (after the first hop) the turn from the previous heading is within `max_turn_angle_deg`. The moment no candidate qualifies, that attempt is scrapped — regardless of whether it died on point 2 or point 4.
* **Short-end reject:** the walk *did* successfully place all 5 points, but the straight-line distance between the first and last point came out below `min_start_end_dist`. The mission is structurally valid but loops back too close to its own start, so it's discarded after the fact instead of mid-build.

Both numbers scale directly with how many `valid_candidates` survive filtering relative to the spacing band you've set. A small candidate pool spread across a wide `min_spacing`–`max_spacing` range means most random walks dead-end almost immediately, and the walks that do survive are more likely to double back and get caught by the short-end check. A high reject count isn't automatically a bug — check the `Valid candidates: N` log line first. If that number is low relative to `candidate_count`, the fix is usually loosening clearance, boundary margin, or spacing ratios rather than raising `max_attempts`.
