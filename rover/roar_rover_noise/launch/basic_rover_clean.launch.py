...
def generate_launch_description():
    ...
    xacro_file = os.path.join(
        pkg_roar_rover_noise, 'urdf', 'base', 'rover_simulation_clean.urdf.xacro')
    ...
    world_file = os.path.join(
        pkg_roar_rover_noise, 'urdf', 'worlds', 'rover_world.sdf')
    ...
    # ── 13. Clean Camera Pass-Through Node ────────────────────────────────────
    node_clean_camera = Node(
        package='roar_rover_noise',
        executable='clean_camera_node.py',
        name='clean_camera_node',
        output='screen'
    )
    # ── 12. Clean Telemetry Node ──────────────────────────────────────────────
    node_clean_joint = Node(
        package='roar_rover_noise',
        executable='clean_joint_state_publisher.py',
        name='clean_joint_state_node',
        output='screen'
    )
