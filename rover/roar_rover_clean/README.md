...
> - **Workspace 2** → ROAR rover simulation (`roar_rover_clean`)
>
...
git clone -b Rover_Arm_Marsyard_Simualtion \
  https://github.com/ASU-ROAR-Team/ROAR-26-Simulation.git roar_rover_clean

# 3. Build it
cd ~/roar_ws
colcon build --packages-select roar_rover_clean
...
# Step 3 — Launch
ros2 launch roar_rover_clean basic_rover_mars.launch.py
```
...
| Arm missing in Gazebo | Rebuild: `colcon build --packages-select roar_rover_clean` |
| `package://roar_rover_clean` not found | Source `~/roar_ws/install/setup.bash` |
