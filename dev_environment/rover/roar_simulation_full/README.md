...
> - **Workspace 2** → ROAR rover simulation (`roar_simulation_full`)
>
...
git clone -b Rover_Arm_Marsyard_Simualtion \
  https://github.com/ASU-ROAR-Team/ROAR-26-Simulation.git roar_simulation_full

# 3. Build it
cd ~/roar_ws
colcon build --packages-select roar_simulation_full
...
# Step 3 — Launch
ros2 launch roar_simulation_full basic_rover_mars.launch.py
```
...
| Arm missing in Gazebo | Rebuild: `colcon build --packages-select roar_simulation_full` |
| `package://roar_simulation_full` not found | Source `~/roar_ws/install/setup.bash` |
